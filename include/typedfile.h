...
bool typedFile<T>::open(const string name, const string type, const unsigned int ver, ios::openmode openmode) {
    ...
    if(!isOpen()){
        ...
        if(isOpen()){
            ...
            //inicializa o cabeçalho e escreve
...
bool typedFile<T>::deleteRecord(unsigned long long int i) {
    ...
    if(i != iPercorre){ //se não estiver no inicio
        while(fstream::good() && iPercorre != 0){
            this->readRecord(anterior, iPercorre); //lê os records anteriores
            if(i == anterior.getNext()){

                //lê o objetivo e movimenta os ponteiros
            ...
            iPercorre = anterior.getNext(); //atualiza a movimentação
        }
...
bool typedFile<T>::readHeader(header &h) {
    if(isOpen())
    {
        ...
        //utilizando a função fromString para atualizar o header
        h.fromString(string(aux, h.size()));

        delete aux; //desalocação de ponteiro
        return true;
    }
    return false;
}
